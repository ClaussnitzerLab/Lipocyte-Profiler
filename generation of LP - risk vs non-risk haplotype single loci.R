...
#####susbet depot
all_data_nonFFA<-subset(merged_locus, merged_locus$FFA == "0")
all_data_FFA<-subset(merged_locus, merged_locus$FFA == "1")
...
####choose which haplotype (number of column) we want to test#####
AP_subset<-cbind(AP_subset[,c(1,8,15:3051)])
#####subset in meta and LP feature data######
meta<-AP_subset[, 1:37]
input<-AP_subset[, 38:3039]
...
case<-subset(df, df$rs6739187 == "AA")

control<-subset(df, df$rs6739187  == "GG")

#####merge case and control###
data_anov<-rbind(case, control)

####transform LP features to numeric and change range depending on input####
data_anov[,38:2720] <- sapply(sapply(data_anov[,38:2720], as.character),as.numeric) 
...
for(i in colvector){
  ...
  lm <- lm(data_anov[,i] ~ rs6739187 +BMI +age +batch + sex , data = data_anov)
...
pdf("risk vs non-risk DNMT3 profile visceral D14 volcano plot.pdf")
plot
dev.off()
######
